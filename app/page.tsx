...
const films = [
  { id: '-ZOak8djgV0', title: 'Live Sessions' },
  { id: 'Ef2hWobQi88', title: 'Music Films' },
  { id: 'SKURpqoLL2Y', title: 'Artist Stories' },
  { id: 'O84e_areg_g', title: 'BES · AI Band Visuals' },
]
...
export default function Page() {
  ...
  return <main className="site-shell">
    ...
    <section className="work-section" id="work"><div className="section-heading"><div><p className="kicker">01 / {t.selected}</p><h2>{t.selected}</h2></div><p>{t.selectedIntro}</p></div><div className="film-grid">{films.map((film, index) => <a className={`film-card card-${index + 1}`} href={`https://youtu.be/${film.id}`} target="_blank" rel="noreferrer" key={film.id}>{film.id === AI_FILM ? <video className="film-preview" autoPlay muted loop playsInline preload="auto" aria-label={`${film.title} — Scheibe Erde`}><source src="/webshow.mp4" type="video/mp4" /></video> : <img className="film-preview" src={`https://i.ytimg.com/vi/${film.id}/maxresdefault.jpg`} alt={`${film.title} — ${t.frameAlt}`} loading="lazy" />}<span className="film-overlay"><small>0{index + 1}</small><strong>{film.title}</strong><em>{film.id === AI_FILM ? t.upcoming : t.watch} ↗</em></span></a>)}</div></section>
    ...
    <section className="services-section" id="services"><div className="section-heading"><div><p className="kicker">03 / {t.services}</p><h2>{t.services}</h2></div><p>{t.serviceText}</p></div><p className="offer-banner">{t.offer}</p><div className="service-list">{t.items.map((item) => <article key={item.n}><span className="service-number">{item.n}</span><h3>{item.title}</h3><p>{item.text}</p><span className="service-price">{item.was && <s>{item.was}</s>}{item.price}</span><span className="service-arrow">↗</span></article>)}</div><p className="price-note">{t.priceNote}</p></section>
    <section className="contact-section" id="contact"><div className="contact-mark"><img src={logo} alt="Scheibe Erde" /></div><p className="kicker">04 / {t.navContact}</p><h2>{t.cta}</h2><p>{t.ctaText}</p><a className="button button-dark" href={`mailto:${EMAIL}`}>{t.write} <span>↗</span></a><p className="contact-name">Chema Gracia</p><p className="contact-email"><a href={`mailto:${EMAIL}`}>{EMAIL}</a></p><p className="contact-email"><a href={`tel:${PHONE_HREF}`}>{PHONE}</a></p></section><footer><span>{t.footer}</span><a href="#top">↑ Back to top</a></footer>
  </main>
}
