...
const copy = {
  de: {
    ...
    items: [['01', 'Live Sessions', 'Mehrspur-Ton, mehrere Kameras, ein fertiger Film.'], ['02', 'Musikvideos', 'Konzept, Dreh und Postproduktion — real, generiert oder dazwischen.'], ['03', 'Content Systeme', 'Reels, EPKs und Bilder, die eure Veröffentlichung tragen.']],
  ...
  en: {
    ...
    services: 'What I do', serviceText: 'A small studio for a strong impact — personal, fast and without unnecessary approval loops.', items: [['01', 'Live Sessions', 'Multitrack sound, multiple cameras, one finished film.'], ['02', 'Music Videos', 'Concept, shoot and post — real, generated or somewhere in between.'], ['03', 'Content Systems', 'Reels, EPKs and images to carry your release.']], cta: 'Let’s make something that lasts.', ctaText: 'Tell me about your next release, your tour, or the idea that has not found its shape yet.', write: 'Write to me', footer: 'Scheibe Erde Media Productions · Berlin · DE / EN / ES', watch: 'Open film on YouTube', upcoming: 'Premiere · Sep 01',
  },
}

const logo = 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/scheibe-erde-logo-8PjWFO5GUlgr0ZWUNV4je0eTz0GRla.png'
const icon = 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/scheibe-erde-icon-T3eFCGMTmQytXyhWaDuBiW2G5QER1O.png'
...
export default function Page() {
  ...
  return <main className="site-shell">
    ...
    <section className="services-section" id="services"><div className="section-heading"><div><p className="kicker">03 / {t.services}</p><h2>{t.services}</h2></div><p>{t.serviceText}</p></div><div className="service-list">{t.items.map((item) => <article key={item[0]}><span className="service-number">{item[0]}</span><h3>{item[1]}</h3><p>{item[2]}</p><span className="service-arrow">↗</span></article>)}</div></section>
    <section className="contact-section" id="contact"><div className="contact-mark"><img src={logo} alt="Scheibe Erde" /></div><p className="kicker">04 / {t.navContact}</p><h2>{t.cta}</h2><p>{t.ctaText}</p><a className="button button-dark" href="mailto:scheibeerdemedia@gmail.com">{t.write} <span>↗</span></a><p className="contact-email">scheibeerdemedia@gmail.com</p></section><footer><span>{t.footer}</span><a href="#top">↑ Back to top</a></footer>
  </main>
}
